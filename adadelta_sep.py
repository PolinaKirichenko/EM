...
class GMM:
    ...
    def Mstep(self, minibatch, gamma, inv, mgrad, mdelta, cgrad, cdelta):
        eps = 1e-6
        ro = 0.95
        ...
        for i in range(self.compnum):
            ...
            cur_mgrad = np.dot(gamma[i] * (minibatch - self.gaussian[i].mu), inv.T)[0]
            mgrad[i] += (1 - ro) * np.dot(cur_mgrad, cur_mgrad)
            cur_mdelta =  math.sqrt(mdelta[i] + eps) / math.sqrt(mgrad[i] + eps) * cur_mgrad
            mdelta[i] = ro * mdelta[i] + (1 - ro) * np.dot(cur_mdelta, cur_mdelta)
            self.gaussian[i].mu += cur_mdelta

            centre = minibatch - self.gaussian[i].mu

            cur_cgrad = (gamma[i] / 2 * (-inv + np.dot(inv, np.dot(centre.T * centre, inv)))).flatten()
            cgrad[i] += (1 - ro) * np.dot(cur_cgrad, cur_cgrad)
            cur_cdelta = math.sqrt(cdelta[i] + eps) / math.sqrt(cgrad[i] + eps) * cur_cgrad
            cdelta[i] = ro * cdelta[i] + (1 - ro) * np.dot(cur_cdelta, cur_cdelta)
            self.gaussian[i].cov += cur_cdelta.reshape((self.dim, self.dim))
    ...
    def stochEM(self, train_set):
        ...
        mgrad = np.zeros(self.compnum)
        mdelta = np.zeros(self.compnum)
        cgrad = np.zeros(self.compnum)
        cdelta = np.zeros(self.compnum)

        freq = 200

        for i in range(2 * tss):
            ...
            self.Mstep(minibatch, gamma, inv, mgrad, mdelta, cgrad, cdelta)
...
def test_report():
    ...
    out = open("report/ada_sep/report.txt", 'a+')
    comp = {1 : 2, 2 : 2, 3 : 3, 4 : 3, 5 : 4, 6: 3, 7: 3, 8 : 3, 9 : 3, 10: 3, 11: 3, 12: 3, 13: 5, 14: 3, 15: 5}
    gnum = comp[setnum]
    obs = np.loadtxt("data/input" + str(setnum))
    ...
    np.savetxt("report/ada_sep/res" + str(setnum), (np.array(means), np.array(covs)))
